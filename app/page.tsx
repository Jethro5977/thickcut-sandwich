...
const pickupOptions = Array.from({ length: 25 }, (_, index) => {
...
function HomeScreen({ onMenu, onOpen }: { onMenu: () => void; onOpen: (id: number) => void }) {
  ...
  return <section className="home-screen">
    ...
    <a className="qr-card" href={assetPath('/images/order-qr.png')} target="_blank"><img src={assetPath('/images/order-qr.png')} alt="本站下单二维码" /><div><small>SCAN TO ORDER</small><strong>把菜单分享给朋友</strong><span>长按保存二维码 · 扫码浏览菜单</span></div><ChevronRight /></a>
  </section>;
}
...
function AboutScreen() { return <section className="about-screen"><div className="about-photo"><img src={assetPath('/images/menu-08.png')} alt="抹茶红豆乳酪三明治" /><span>从一份认真称重的早餐开始</span></div><div className="page-title"><small>ABOUT FAFA</small><h1>FaFa 的坚持</h1></div><p className="about-copy">从家庭厨房开始，八款配方反复调整。每一种食材上秤称重后才夹进面包——所以我们敢把克重和热量都写在菜单上。</p><div className="values"><article><span><Wheat /></span><div><h2>称重出品</h2><p>食材按配方称重，热量误差尽量控制在 ±10% 内。</p></div></article><article><span><Leaf /></span><div><h2>清爽酱料</h2><p>自制低卡沙拉酱与蜂蜜芥末，甜系使用代糖。</p></div></article><article><span><Sparkles /></span><div><h2>当日现做</h2><p>不隔夜、不预制，确认预约后再组装。</p></div></article></div><div className="contact-card"><img src={assetPath('/images/order-qr.png')} alt="菜单二维码" /><div><small>CONTACT US</small><strong>微信：TwiceTwice1 / DoncicX</strong><span>营业 09:00–21:00<br />预约自取 · 3 公里内配送</span></div></div><p className="estimate-note">孕期、过敏或特殊饮食需求请提前告知</p></section>; }
